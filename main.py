...
class GUI:
    def __init__(self,menuitem,id):
        ...
        self.padx=70
        self.pady=40
        self.attribute=[Label(self.mainframe,text=(str(x)).capitalize()+":",wraplength=500, justify= LEFT, font=('Helvetica', 8, 'bold')).grid(row =r, column =c,sticky='ew',padx= self.padx) for x,r,c in zip(self.menuitem[0],self.row,self.col)] 
        ...
        self.listbox = Listbox(self.selectbox,listvariable=StringVar(value=self.id),height=24,selectmode='extended')
    ...
    def UpdateMainFrame(self,event):
        ...
        self.mainframe.grid(column=1,row=1)
        self.mainframe.grid_propagate(0)
        self.attribute=[Label(self.mainframe,text=str(x).capitalize()+":",wraplength=500, justify= LEFT, font=('Helvetica', 8, 'bold')).grid(row =r, column =c,sticky='ew',padx= self.padx) for x,r,c in zip(self.menuitem[self.choice],self.row,self.col)] 
